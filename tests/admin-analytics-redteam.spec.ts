...
test.describe('G001 관리자 분석 레드팀', () => {
  ...
  test('dashboard 500은 오류를 표시하고 앱을 크래시하지 않는다', async ({ page }) => {
    ...
    await expect(page.getByText('분석 데이터를 불러오지 못했습니다.')).toBeVisible();
  ...
  test('빠른 기간 전환은 늦은 이전 응답을 새 탭에 표시하지 않는다', async ({ page }) => {
    ...
    await weekSeen;
    await page.getByLabel('분석 기간').getByRole('button', { name: '최근 30일', exact: true }).click();
    await expect(page.getByLabel('분석 기간').getByRole('button', { name: '최근 30일', exact: true })).toHaveAttribute('aria-pressed', 'true');
    await expect(page.getByLabel('선택 기간 요약').getByText('29,029건', { exact: true })).toBeVisible();
    releaseWeek();
    await page.waitForTimeout(200);
    await expect(page.getByLabel('선택 기간 요약').getByText('70,007건', { exact: true })).toHaveCount(0);
  });

  test('1023px와 1024px 모두 분석 현황을 조회한다', async ({ browser }) => {
    ...
    expect(narrowDashboard).toBeGreaterThan(0);
    await narrow.close();
  ...
  test('실제 로그인 화면 증거를 저장한다', async ({ page }) => {
    ...
    await page.goto('/admin/analytics/dashboard');
    await expect(page.getByRole('heading', { name: '분석 현황' })).toBeVisible();
    await expect(page.getByRole('region', { name:'현재 남은 업무' })).toBeVisible();
