...
export default function AnalyticsRatings() {
  const { data, error, lastUpdatedAt } = usePolling<{ ranking: RatingRanking[] }>('/api/admin/analytics/ratings', 45_000);
  ...
  const { data: detail, error: detailError } = usePolling<Detail>(
    selectedKey ? `/api/admin/analytics/ratings/${encodeURIComponent(selectedKey)}` : null,
    45_000,
  );
  ...
  const columns: Column<RatingRanking, 'rank' | 'name' | 'type' | 'rating' | 'reviewCount' | 'completed'>[] = [
    ...
    { key: 'type', label: '유형', sortable: true, sortValue: (row) => row.type, render: (row) => row.type === 'PROVIDER' ? '업체' : '전기기사' },
