...
// 로그인한 기사가 자기 정보(전화·주소·서비스지역·영업상태)를 조회/수정한다.
// 기사명·아이디·사업자번호 같은 신원 정보는 관리자만 바꿀 수 있어 여기서 제외한다.
...
export async function GET() {
  ...
  const p = await prisma.technician.findUnique({
...
export async function PATCH(req: NextRequest) {
  ...
  if (data.address !== undefined && data.address !== technician.address) {
    technicianData.address = data.address;
    // 주소가 바뀌면 좌표를 다시 지오코딩 (실패 시 좌표 없음 — 관리자가 보완)
    const geo = await geocode(data.address);
    technicianData.lat = geo?.lat ?? null;
    technicianData.lng = geo?.lng ?? null;
  }
  if (data.regions !== undefined)
    technicianData.regions = sanitizeRegionKeys(data.regions);
