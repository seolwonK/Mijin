...
//
// provider 3종:
//   kcp     NHN KCP 본인확인(V2) 직접 연동 — 운영 기본. 브라우저는 공개 키가 필요 없고(서버가 거래등록을
//           대신 하고 ordr_idxx 만 내려준다) 비밀(KCP_ENC_KEY)은 서버에만 있다(src/lib/identity/kcp/).
//   portone 포트원 V2 경유(KCP 채널). 포트원 실연동은 요금 결제 카드 등록이 필수라 2026-09 부터 쓰지 않는다.
//   mock    개발용 — 입력값을 그대로 인증 처리.
//
...
export type IdentityProviderName = 'kcp' | 'portone' | 'mock';
...
export function identityProviderName(): IdentityProviderName {
  const raw = process.env.IDENTITY_PROVIDER?.trim();
  if (raw === 'kcp') return 'kcp';
  if (raw === 'portone') return 'portone';
  ...
  if (
    ...
    throw new Error(
      '본인인증 설정 오류: 프로덕션에서는 IDENTITY_PROVIDER=kcp (또는 IDENTITY_PROVIDER=portone) 이어야 합니다 (mock 은 무인증 가입을 허용합니다)',
    );
  }
  return 'mock';
}
...
export function getIdentityPublicConfig(): IdentityPublicConfig {
  ...
  if (name === 'mock') return { provider: 'mock' };
  // kcp 는 브라우저에 내려줄 식별자가 없다 — 거래등록은 서버(/api/identity/kcp/start)가 하고,
  // 사이트코드·ENC_KEY 는 서버 비밀이다. 설정 누락은 그 라우트가 호출될 때 드러난다.
  if (name === 'kcp') return { provider: 'kcp' };
